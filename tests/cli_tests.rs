...
fn add_dry_run_json_is_valid_json() {
    ...
    let parsed: serde_json::Value = serde_json::from_str(&stdout)
        .expect("dry-run output should be valid JSON");
...
fn init_local_creates_env_file() {
    ...
    assert!(content.contains("TICKTICK_CLIENT_ID"), ".env should contain CLIENT_ID template");
    assert!(content.contains("TICKTICK_CLIENT_SECRET"), ".env should contain CLIENT_SECRET template");
}
...
fn init_local_force_overwrites() {
    ...
    let content = fs::read_to_string(&env_path).unwrap();
    assert!(content.contains("TICKTICK_CLIENT_ID"), "should be overwritten with template");
}
...
fn errors_are_json_on_stderr() {
    ...
    if !output.status.success() {
        ...
        let parsed: serde_json::Value = serde_json::from_str(stderr.trim())
            .expect("error output should be valid JSON");
        assert!(parsed["error"].is_string(), "error JSON should have 'error' field");
    }
}
